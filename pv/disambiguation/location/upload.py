...
def create_tables():
    ...
    pg_cursor.execute(
        "CREATE TABLE tmp_location_disambiguation_pregranted (uuid VARCHAR(255), disambiguated_id VARCHAR(255))")
    g_cursor.close()
    pg_cursor.close()
...
def drop_tables():
    ...
    pg_cursor = cnx_pg.cursor()
    pg_cursor.execute("DROP TABLE tmp_location_disambiguation_pregranted")
    g_cursor.close()
    pg_cursor.close()
...
def upload(granted_ids, pregranted_ids):
    ...
    for idx in tqdm(range(len(offsets)), 'adding granted', total=len(offsets)):
        ...
        sql = "INSERT INTO tmp_location_disambiguation_granted (uuid, disambiguated_id) VALUES " + ', '.join(
            ['("%s", "%s")' % x for x in pairs_granted[sidx:eidx]])
        # logging.log_first_n(logging.INFO, '%s', 1, sql)
        g_cursor.execute(sql)
    cnx_g.commit()
    g_cursor.execute('alter table tmp_location_disambiguation_granted add primary key (uuid)')
    cnx_g.close()
    ...
    for idx in tqdm(range(len(offsets)), 'adding pregranted', total=len(offsets)):
        ...
        sql = "INSERT INTO tmp_location_disambiguation_pregranted (uuid, disambiguated_id) VALUES " + ', '.join(
            ['("%s", "%s")' % x for x in pairs_pregranted[sidx:eidx]])
        # logging.log_first_n(logging.INFO, '%s', 1, sql)
        pg_cursor.execute(sql)
    cnx_pg.commit()
    pg_cursor.execute('alter table tmp_location_disambiguation_pregranted add primary key (uuid)')
    cnx_pg.close()
